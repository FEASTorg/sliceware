...
void loop()
{
  ...
  // FOR PUMP 1 PULSING
  if (pwm1 != 0)
  { // only run if duty cycle is nonzero
    int onTime1 = period * pwm1 / 255;
    int offTime1 = period - onTime1;
    if (!pump1On && millis() - curTime1 >= offTime1)
    { // wait for off cycle to finish
      curTime1 = millis();
      digitalWrite(DIR1, dir1);
      analogWrite(MC1, 160); // minimum speed to turn pump
      pump1On = true;
    }
    else if (pump1On && millis() - curTime1 >= onTime1)
    { // wait for on cycle to finish
      curTime1 = millis();
      digitalWrite(DIR1, 1);
      analogWrite(MC1, 0);
      pump1On = false;
  ...
  { // only run if duty cycle is nonzero
    ...
    if (!pump2On && millis() - curTime2 >= offTime2)
    { // wait for off cycle to finish
      curTime2 = millis();
    ...
    else if (pump2On && millis() - curTime2 >= onTime2)
    { // wait for on cycle to finish
      curTime2 = millis();
      digitalWrite(DIR2, 1);
      analogWrite(MC2, 0);
